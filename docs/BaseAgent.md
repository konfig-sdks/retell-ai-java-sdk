...
|------------ | ------------- | ------------- | -------------|
|**llmWebsocketUrl** | **String** | The URL we will establish LLM websocket for getting response, usually your server. Check out [LLM WebSocket](https://raw.githubusercontent.com) for more about request format (sent from us) and response format (send to us). |  [optional] |
|**voiceId** | **String** | Unique voice id used for the agent. Find list of available voices and their preview in Dashboard. |  [optional] |
